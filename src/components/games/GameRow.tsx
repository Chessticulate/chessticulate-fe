...
export default function GameRow({ game, setGame, setGameTab }: Props) {
  const token = getCookie("token") as string;
  const decodedToken = jwtDecode<Jwt>(token);
  const { id, white, black, white_username, black_username, winner } = game;

  const perspective =
    decodedToken.user_name === white_username ? "white" : "black";
